...
export function UsersPage() {
  ...
  // ⚡ Bolt: Optimize user stats calculation
  // Replaced multiple O(N) array filter/length operations with a single O(N) reduce pass
  ...
  const stats = useMemo(() => {
    ...
    return data.data.reduce(
      (acc, user) => {
        ...
        if (user.status === UserStatus.INACTIVE) acc.inactive++;
        return acc;
      },
